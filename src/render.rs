...
use crate::{types, utils::path_to_http_url};
...
impl Paginator {
    pub fn new(per_page: u32, item_count: usize) -> Self {
        PAGE_PERMALINK.with_borrow(|page_permalink| {
            ...
            Paginator {
                ...
                page_permalinks: (0u32..=last_page)
                    .map(|page| path_to_http_url((*page_permalink)(page)).unwrap())
                    .collect(),
            }
        })
    }
...
fn paginate(items: &minijinja::Value, per_page: u32) -> Result<minijinja::Value, minijinja::Error> {
    PAGINATOR.with_borrow_mut(|paginator| {
        if paginator.is_none() {
            ...
            *paginator = Some(Paginator::new(per_page, item_count));
        }
...
impl Renderer {
    pub fn build(base_url: String, template_path: impl AsRef<Path>) -> anyhow::Result<Renderer> {
        ...
        t.add_function("paginate", paginate);
...

        {
            ...
            t.add_filter(
                ...
                move |path: ViaDeserialize<PathBuf>| -> Result<String, minijinja::Error> {
                    ...
                    url.push_str(&root_url);
                    for part in path.iter() {
                        url.push('/');
                        url.push_str(part.to_str().ok_or_else(|| {
                            minijinja::Error::new(
                                minijinja::ErrorKind::InvalidOperation,
